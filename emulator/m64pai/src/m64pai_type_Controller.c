...
/* ------------------------------------------------------------------------- */
static void
Controller_dealloc(m64pai_Controller* self)
{
    Py_XDECREF(self->emu);
...
{
    m64pai_Controller* self;

    self = (m64pai_Controller*)type->tp_alloc(type, 0);
    if (self == NULL)
        goto alloc_self_failed;

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!i", kwds_names, &m64pai_EmulatorType, &self->emu, &self->player_slot))
...
/* ------------------------------------------------------------------------- */
static m64pai_CuckedInputPlugin*
try_get_cucked_input(m64pai_Controller* self)
{
    if (!m64pai_CuckedInputPlugin_CheckExact(self->emu->input_plugin))
    {
        if (m64pai_Plugin_CheckExact(self->emu->input_plugin))
    ...
    return (m64pai_CuckedInputPlugin*)self->emu->input_plugin;
}
...
"by a human player.");
static PyObject*
Controller_getoverride(m64pai_Controller* self, void* closure)
{
    m64pai_CuckedInputPlugin* cuck = try_get_cucked_input(self);
    if (cuck == NULL)
        return NULL;
...
static int
Controller_setoverride(m64pai_Controller* self, PyObject* value, void* closure)
{
    int trueness;
    m64pai_CuckedInputPlugin* cuck = try_get_cucked_input(self);
    if (cuck == NULL)
...
#define X(mupen, python, desc)                                                \
    ...
    static PyObject* Controller_get##python(m64pai_Controller* self, void* closure) { GET_BUTTON(mupen); } \
    static int Controller_set##python(m64pai_Controller* self, PyObject* value, void* closure) { SET_BUTTON(mupen); }
GETSET_BUTTONS_LIST
#undef X
...
static PyObject*
Controller_getx(m64pai_Controller* self, void* closure)
{
    int8_t x_axis;
    m64pai_CuckedInputPlugin* cuck = try_get_cucked_input(self);
    if (cuck == NULL)
...
static int
Controller_setx(m64pai_Controller* self, PyObject* py_value, void* closure)
{
    double value;
    BUTTONS buttons;
    m64pai_CuckedInputPlugin* cuck = try_get_cucked_input(self);
...
"to -127 to 127 before converting it to a float.");
static PyObject*
Controller_gety(m64pai_Controller* self, void* closure)
{
    int8_t y_axis;
    m64pai_CuckedInputPlugin* cuck = try_get_cucked_input(self);
    if (cuck == NULL)
...
static int
Controller_sety(m64pai_Controller* self, PyObject* py_value, void* closure)
{
    double value;
    BUTTONS buttons;
    m64pai_CuckedInputPlugin* cuck = try_get_cucked_input(self);
...
PyTypeObject m64pai_ControllerType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "m64pai.Controller",
    .tp_basicsize = sizeof(m64pai_Controller),
...
{
    if (PyType_Ready(&m64pai_ControllerType) < 0)
        return -1;
    return 0;
}
